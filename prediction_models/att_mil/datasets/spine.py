...
def tile(img, mask, location, iou, sz=256, N=36, scale = 8):
    ...
    for i in range(min(N, len(idxsort))):
        ...
        if shape[0] < sz or shape[1] < sz:
            ...
            if mask:
                warped_mask = np.pad(warped_mask, [[pad0 // 2, pad0 - pad0 // 2], [pad1 // 2, pad1 - pad1 // 2], [0, 0]],
                                     mode='constant', constant_values=0)
            warped_img = np.pad(warped_img, [[pad0 // 2, pad0 - pad0 // 2], [pad1 // 2, pad1 - pad1 // 2], [0, 0]],
                                mode='constant', constant_values=255)
        cur_info = {'img': warped_img[:sz, :sz, :],  'location': cnt, 'orig_location': location[idx], 'idx': idx}
        if mask:
            cur_info['mask'] = warped_mask[:sz, :sz, :],
        result.append(cur_info)
    if len(idxsort) < N:
        for i in range(N - len(idxsort)):
            cur_info = {'img': np.zeros((sz, sz, 3)).astype(np.uint8), 'location': None, 'orig_location': None,
                        'idx': i}
            if mask:
                cur_info['mask'] = np.zeros((sz, sz, 3)).astype(np.uint8)
            result.append(cur_info)
    return result
