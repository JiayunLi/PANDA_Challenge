...
def loss_xent(logits, labels, ignore_index=-1, n_classes=1):
    '''
    compute multinomial cross-entropy, for e.g. training a classifier.
    '''
    if n_classes == 1:
        xent = F.binary_cross_entropy_with_logits(logits, torch.unsqueeze(labels.float(), dim=1))
    else:
...
class NCE_MI(nn.Module):
    def __init__(self, tclip=20.,device='cpu'):
        super(NCE_MI, self).__init__()
        self.tclip = tclip
        self.device = device
    ...
    def _model_scores(self, r_src, r_trg, mask_mat):
        ...
        raw_scores = raw_scores.reshape(n_batch, n_batch, n_locs)
    ...
    def forward(self, r1_src_1, r5_src_1, r1_src_2, r5_src_2, r5_trg_1, r5_trg_2, mask_mat, mode,
                r7_trg_1=None, r7_trg_2=None):
        ...
        if mode == 'train':
            ...
            if mask_mat.sum().item() < 1e-1:
                ...
                loss_1t5_1, lgt_1t5_1 = self._loss_g2l(r1_src_1, r5_trg_2, mask_mat)
                loss_1t5_2, lgt_1t5_2 = self._loss_g2l(r1_src_2, r5_trg_1, mask_mat)

                # combine costs for optimization
                loss_1t5 = 0.5 * (loss_1t5_1 + loss_1t5_2)

                lgt_reg = 0.5 * (lgt_1t5_1 + lgt_1t5_2)
            return loss_1t5, lgt_reg
        else:
            # compute values to use for visualizations
            nce_scores, raw_scores, lgt_reg = \
                self._model_scores(r1_src_1, r5_trg_2, mask_mat)
            return nce_scores, raw_scores


class LossNCE(nn.Module):
    '''
    Input is fixed as r1_x1, r5_x1, r7_x1, r1_x2, r5_x2, r7_x2.
    '''

    def __init__(self, tclip=10., device='cpu'):
        super(LossNCE, self).__init__()
        # initialize the dataparallel nce computer (magic!)
        self.device = device
        self.nce_func = self._config_nce(tclip)
        # self.nce_func = nn.DataParallel(self.nce_func)
        # construct masks for sampling source features from 5x5 layer, which 1 from 5*5 feature map to be sampled
    ...
    def _config_nce(self, tclip):
        nce_func = NCE_MI(tclip=tclip, device=self.device)
        # initialize the dataparallel nce computer (magic!)
        # self.nce_func = nn.DataParallel(self.nce_func)
        return nce_func
    ...
    def forward(self, r1_x1, r5_x1, r1_x2, r5_x2, mode='train', r7_x1=None, r7_x2=None):
        '''
        ...
        mask_mat = torch.eye(n_batch).to(self.device)
        ...
        loss_1t5, lgt_reg = \
            self.nce_func(r1_src_1, r5_src_1, r1_src_2, r5_src_2,
                          r5_trg_1, r5_trg_2, mask_mat, mode='train')
...
class NCE_MI_MULTI(NCE_MI):
    def __init__(self, tclip=20., device='cpu'):
        super().__init__(tclip, device)
    ...
    def forward(self, r1_src_1, r5_src_1, r1_src_2, r5_src_2, r5_trg_1, r5_trg_2, mask_mat, mode,
        ...
        if mode == 'train':
            ...
            if mask_mat.sum().item() < 1e-1:
                ...
                loss_1t7_1, lgt_1t7_1 = self._loss_g2l(r1_src_1, r7_trg_2, mask_mat)
                loss_1t7_2, lgt_1t7_2 = self._loss_g2l(r1_src_2, r7_trg_1, mask_mat)
                # compute costs for 5->5 prediction
                loss_5t5_1, lgt_5t5_1 = self._loss_g2l(r5_src_1, r5_trg_2, mask_mat)
                loss_5t5_2, lgt_5t5_2 = self._loss_g2l(r5_src_2, r5_trg_1, mask_mat)
                # combine costs for optimization
                loss_1t5 = 0.5 * (loss_1t5_1 + loss_1t5_2)
...
class LossMultiNCE(LossNCE):
    def __init__(self, tclip=10., device='cpu'):
        super().__init__(tclip, device)

    def _config_nce(self, tclip):
        nce_func = NCE_MI_MULTI(tclip=tclip, device=self.device)
        # initialize the dataparallel nce computer (magic!)
        # self.nce_func = nn.DataParallel(self.nce_func)
        return nce_func
    ...
    def forward(self, r1_x1, r5_x1, r1_x2, r5_x2, mode='train', r7_x1=None, r7_x2=None):
        ...
        # compute nce for multiple infomax costs across multiple GPUs
        # -- we do some hacky stuff to minimize compute/mem costs for cuda:

        loss_1t5, loss_1t7, loss_5t5, lgt_reg = \
            self.nce_func(r1_src_1, r5_src_1, r1_src_2, r5_src_2,
                          r5_trg_1, r5_trg_2, mask_mat, mode='train',
                          r7_trg_1=r7_trg_1, r7_trg_2=r7_trg_2)
        loss_1t5 = loss_1t5.mean()
