...
def train_epoch(epoch, iteras, model, slide_criterion, tile_criterion, optimizer,
    ...
    for step in range(len(train_loader)):
        ...
        tile_labels = torch.squeeze(tile_labels, dim=0)
        ...
        optimizer.step()
        cur_dict = {
            'slide_loss': slide_loss.item().cpu(),
            'tot_loss': loss.item().cpu(),
        }
        if tile_labels[0] != -1:
            cur_dict['tile_loss'] = tile_loss.item().cpu()
...
def trainval(fold, exp_dir, start_epoch, iters, trainval_params, model, optimizer, scheduler,
    ...
    logger = trainval_stats.StatTracker(log_dir=f"{exp_dir}/{fold}/")
    ...
    for epoch in range(start_epoch, trainval_params.tot_epochs):
        print(f"Start training for Fold {fold}\t Epoch: {epoch}/{len(trainval_params.tot_epochs)}")
        if epoch == trainval_params.feat_ft:
            optimizer, scheduler = \
        ...
        kappa, loss = val(epoch, model, val_loader, slide_criterion, logger)
...
def val(epoch, model, val_loader, slide_criterion, logger):
    model.eval()
    torch.cuda.empty_cache()
    # Quick check of status for log_every steps
