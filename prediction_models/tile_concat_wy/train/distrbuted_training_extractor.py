...
from model.resnext_ssl_extractor import *
...
if __name__ == "__main__":
    ...
    fname = f'Resnext50_{N}patch_br_extractor_{provider}'
    if provider == "rad":
    ...
    for fold in folds:
        ...
        for epoch in tqdm(range(start_epoch,epochs), desc='epoch'):
            ...
            if args.local_rank == 0:
                ...
                writer.flush()
                tqdm.write("Epoch {}, train loss: {:.4f}, val loss: {:.4f}\n".format(epoch,
                                                                                   train['train_loss'],
                                                                                   val['val_loss']))
            ## save the checkpoints and best model
            if args.local_rank == 0:
                is_best = val['val_loss'] < best_val_loss
                save_checkpoint({
                    'epoch': epoch,
                    'state_dict': model.state_dict(),
                    'optimizer': optimizer.state_dict(),
                }, is_best, weightsPath)
                best_val_loss = val['val_loss'] if is_best else best_val_loss
