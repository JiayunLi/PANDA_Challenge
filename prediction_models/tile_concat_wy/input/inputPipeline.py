...
class PandaPatchDatasetSeg(Dataset):
    """Panda Tile dataset. With fixed tiles for each slide."""
    def __init__(self, csv_file, image_dir, mask_dir, N = 12, transform=None):
        """
        Args:
            csv_file (string): Path to the csv file with annotations.
            image_dir (string): Directory with all the images.
    ...
    def __getitem__(self, idx):
        ...
        maskfnames = [os.path.join(self.mask_dir, self.train_csv.loc[idx, 'image_id']+'_'+str(i)+'.png')
                  for i in range(self.N)]
        imgs = [self.open_image(fname) for fname in imgfnames]
        ...
        imgs = torch.stack(imgs)
        # isup_grade = torch.tensor(isup_grade)
        sample = {'image': imgs, 'isup_grade': masks}
        return sample
...
class PandaPatchDatasetInfer(Dataset):
    ...
    def tile_image(self, img):
        ...
        if len(img) < self.N:
            img = np.pad(img, [[0, self.N - len(img)], [0, 0], [0, 0], [0, 0]], mode='constant', constant_values=255)
