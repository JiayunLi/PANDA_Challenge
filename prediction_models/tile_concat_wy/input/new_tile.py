...
def _mask_tissue(image, kernel_size=(7, 7), gray_threshold=220):
    ...
    _,contour, _ = cv2.findContours(mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contour:
...
def tile(img, mask, coords, sz=256):
    result = []
    ...
    for i in range(len(coords)):
        ...
        if shape[0] < sz or shape[1] < sz:
            ...
            this_mask = np.pad(this_mask, [[pad0 // 2, pad0 - pad0 // 2], [pad1 // 2, pad1 - pad1 // 2], [0, 0]], mode='constant',
